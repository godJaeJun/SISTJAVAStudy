...
○ Serializable 인터페이스
   ...
   - 객체 직렬화를 하기 위해 먼저 객체 직렬화가 가능하도록
	 ...
	 ObjectInputStream 클래스와 ObjetOutputStream 클래스를 이용하여
	 객체 단위로 입출력을 수행하게 된다.
...
public class Test171
{
	public static void main(String[] args) throws Exception
	{
		...
		// 결과적으로 『C:\JavaStudy\JavaStudy\data\test.ser』
		
		// 확인
		//System.out.println(f0.getParentFile().exists());
		//--==>> false -> 디렉터리 구조가 존재하지 않는다.
		...
		if (f0.exists())	// f0 파일 객체가 존재한다면...
		{
			...
			{
				//key = (String)e.nextElements());
				//System.out.pritln(key);

				key = (String)e.nextElement();	//key값을 읽어들인다.
